...
void main(void){
    ...
    int cpuid;
    if (MEM_BASE == 0x20000000) {
        cpuid = 0;
    } else if (MEM_BASE == 0x30000000) {
        cpuid = 1;
    } else if (MEM_BASE == 0x35000000) {
        cpuid = 2;
    } else if (MEM_BASE == 0x3A000000) {
        cpuid = 3;
    }
    ...
    if (cpuid != 0){
        ...
        for(int a_len = 1; a_len < 32; a_len++) { 
            ...
            #ifdef CUA_RD
            asm volatile ("interfering_core_prime:\n"
                            "mov x2, #0\n"
                        "intf_repeat_loop_prime:\n"
                            "mov x0, %[array]\n"            // Load the base address of the array into register x0
                            "mov x1, %[array_end]\n"        // Load the end address of the array into register x1
                            #ifdef POINTER_CHASING
                            "ldr x3, [x0], #64\n"            // Load and increment by 64 bytes
                        ...
                        "intf_loop_start_prime:\n"
                            ...
                            #endif
                            "cmp x0, x1\n"                  // Compare the current address with the end address
                            "b.lt intf_loop_start_prime\n"             // Branch back to the start of the loop if less than
                            "add x2, x2, #1\n"
                            "cmp x2, #1\n"               // cmp x2, #<number_of_reps>
                            "b.lt intf_repeat_loop_prime\n"
                            : : [array] "r" (data_array), [array_end] "r" (data_array_end)
                            : "cc", "x0", "x1", "x2", "x3" 
            );
            #endif
            ...
            // Repetition implemented within the assembly
            // Pointer chasing not implemented for assembly
            #ifdef CUA_RD
            ...
            printf("Time spent for size: %0.0f KB, per read/wr @100mhz is: %0.2f\n", (int)(eval_array[a_len]*8.0)/1024.0, ((float)((end-start)*8)/((float)(100*eval_array[a_len]))));
            printf("Total Read/Write is: %d\n", (eval_array[a_len]*8)/64);
            ...
            printf("Time taken in 100Mhz timer cycles %llu\r\n", end-start);
